...
export function activate(context: vscode.ExtensionContext) {
  function createButtons() {
    ...
    commands.forEach((cmdConfig, index) => {
      ...
      const alignment = cmdConfig.alignment === 'right' ? vscode.StatusBarAlignment.Right : vscode.StatusBarAlignment.Left;
      const priority = cmdConfig.priority || 0;
      const statusBarItem = vscode.window.createStatusBarItem(alignment, priority);
      statusBarItem.command = commandId;
      statusBarItem.text = cmdConfig.icon ? `$(${cmdConfig.icon}) ${cmdConfig.text}` : cmdConfig.text;
        ...
        function renderButtons() {
          ...
          commands.forEach((cmd, index) => {
            ...
            div.innerHTML = \`
              ...
              <label>Icon: <input type="text" data-index="\${index}" data-field="icon" value="\${cmd.icon || ''}" /></label>
